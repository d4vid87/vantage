...
/**
 * VANTAGE — JSON fetch over Node's https client.
 *
 * Some upstreams VANTAGE depends on cannot be reached with the bundled undici
...
export const VANTAGE_UA = 'VANTAGE-OSINT/1.0 (+https://github.com/d4vid87/vantage)';
...
function request(url: string, { timeoutMs = 20000, headers = {} }: RequestOptions): Promise<RawResponse> {
  return new Promise<RawResponse>((resolve, reject) => {
    const req = https.get(
      url,
      {
        headers: { 'User-Agent': VANTAGE_UA, Accept: 'application/json', 'Accept-Language': 'en', ...headers },
        timeout: timeoutMs,
      },
      (res) => {
