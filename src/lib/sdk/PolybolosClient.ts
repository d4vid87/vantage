 ...
 * ║                                                                 ║
 * ║  Subscribes to VANTAGE feeds + external providers (Lattice),     ║
...
// ── VANTAGE Feed → Entity Translators ───────────────────────────────
...
function translateFlights(flights: any[], subtype: string): PolybolosEntity[] {
  ...
  return flights.map((f: any) => ({
    id: `vantage-air-${f.icao24 || f.callsign || Math.random().toString(36).slice(2)}`,
    ...
    source: { provider: 'vantage', feed: `flights-${subtype}`, originalId: f.icao24, confidence: 0.9 },
...
function translateMaritime(ships: any[]): PolybolosEntity[] {
  ...
  return ships.map((s: any) => ({
    id: `vantage-sea-${s.mmsi || s.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'maritime-ais', originalId: s.mmsi?.toString(), confidence: 0.85 },
    timestamp: new Date().toISOString(),
...
function translateEarthquakes(events: any[]): PolybolosEntity[] {
  ...
  return events.map((eq: any) => ({
    id: `vantage-event-eq-${eq.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'usgs-earthquakes', originalId: eq.id, confidence: 0.99 },
    timestamp: new Date().toISOString(),
...
function translateSatellites(sats: any[]): PolybolosEntity[] {
  ...
  return sats.map((s: any) => ({
    id: `vantage-space-${s.noradId || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'satnogs', originalId: s.noradId?.toString(), confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
function translateFires(fires: any[]): PolybolosEntity[] {
  ...
  return fires.map((f: any, i: number) => ({
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'nasa-firms', confidence: 0.9 },
    timestamp: new Date().toISOString(),
...
function translateCCTV(cameras: any[]): PolybolosEntity[] {
  ...
  return cameras.map((c: any) => ({
    id: `vantage-sensor-cctv-${c.id || Math.random().toString(36).slice(2)}`,
    name: c.name || 'Camera',
    domain: Domain.LAND,
    ...
    threat: ThreatLevel.NONE,
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'cctv-network', originalId: c.id, confidence: 1.0 },
    timestamp: new Date().toISOString(),
...
function translateRadiation(stations: any[]): PolybolosEntity[] {
  ...
  return stations.map((r: any) => ({
    id: `vantage-sensor-rad-${r.name || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'vantage', feed: 'radiation-network', confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
export class PolybolosClient {
  ...
  /** Connect to the VANTAGE SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      this.sseConnection = new EventSource(
        `${this.config.vantageBaseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw VANTAGE data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestVantageData(data: Record<string, any>): void {
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count VANTAGE feeds that have data
