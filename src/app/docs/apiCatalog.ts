...
export const API_GROUPS: ApiGroup[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        method: 'GET',
        summary: 'PM2.5 readings for major population centres.',
        returns: ['stations', 'total', 'source', 'timestamp'],
        env: ['OPENAQ_API_KEY'],
        notes: 'Keyless via Open-Meteo over a 116-city grid (OpenAQ v2 is retired and v3 needs a key). Set OPENAQ_API_KEY to use OpenAQ ground stations instead.',
      },
      {
        path: '/api/radar',
        method: 'GET',
        summary: 'Macroscopic internet outages detected by IODA.',
        returns: ['outages', 'total', 'source', 'timestamp'],
        notes: 'Georgia Tech IODA, keyless. Merged with Cloudflare Radar in the Internet Disruptions layer when CLOUDFLARE_API_TOKEN is set.',
      },
      {
        path: '/api/radiation',
        method: 'GET',
        summary: 'Radiation readings from the Safecast sensor network.',
        returns: ['stations', 'total', 'source', 'timestamp'],
        notes: 'Safecast, CC0. Readings from the trailing 48h, converted to microsieverts/hour and reduced to the highest reading per ~1km bucket.',
      },
      {
        path: '/api/balloons',
        method: 'GET',
        summary: 'Weather balloons and radiosondes in flight.',
        returns: ['balloons', 'total', 'source', 'timestamp'],
        notes: 'SondeHub, keyless. Flight phase falls back to altitude when a sonde reports no vertical velocity.',
      },
      {
        path: '/api/volcanoes',
        method: 'GET',
        summary: 'Weekly volcanic activity report.',
        returns: ['volcanoes', 'total', 'source', 'timestamp'],
        notes: 'Smithsonian GVP / USGS GeoRSS. Requires an XML Accept header upstream — the feed 403s otherwise.',
      },
      {
        path: '/api/disease',
        method: 'GET',
        summary: 'Disease outbreaks from WHO Disease Outbreak News.',
        returns: ['outbreaks', 'total', 'source', 'timestamp'],
        notes: 'Country is parsed from the headline, and repeated updates of one outbreak collapse to a single entry placed on the country centroid.',
      },
      {
        path: '/api/power-outages',
        method: 'GET',
        summary: 'US power outages by county.',
        returns: ['outages', 'total', 'customers_affected', 'source', 'timestamp'],
        notes: 'DOE / Oak Ridge ODIN, keyless. US coverage only, limited to utilities that report into ODIN.',
      },
      {
        path: '/api/sentinel',
        method: 'GET',
  ...
  },
  {
    ...
    endpoints: [
      {
        path: '/api/travel-advisories',
        method: 'GET',
        summary: 'US State Department travel advisories, levels 1-4.',
        returns: ['advisories', 'total', 'source', 'timestamp'],
        notes: 'Country is resolved from the headline: the feed\'s Category field carries FIPS/GEC codes ("UP" for Ukraine), not ISO 3166. Drives a country choropleth.',
      },
      {
        path: '/api/gps-jamming',
        method: 'GET',
        summary: 'GPS/GNSS interference as H3 cell polygons.',
        returns: ['cells', 'total', 'day', 'source', 'timestamp'],
        notes: 'gpsjam.org daily aircraft-reported navigation accuracy. Cells with fewer than five aircraft are dropped; H3 decoding happens server-side. Attribution requested by the source.',
      },
      {
        path: '/api/acled',
        method: 'GET',
        summary: 'Armed conflict and protest events from ACLED.',
        params: [{ name: 'probe', desc: 'Set to 1 to check configuration without fetching.', example: '1' }],
        returns: ['configured', 'events', 'total', 'source', 'timestamp'],
        env: ['ACLED_API_KEY', 'ACLED_EMAIL'],
        requiresAuth: true,
        notes: 'Free registration required; both variables must be set or the route answers 503 and the layer stays hidden. Attribution required by ACLED terms.',
      },
      {
        path: '/api/polymarket',
        method: 'GET',
        summary: 'Geopolitical prediction-market odds as an early-warning signal.',
        returns: ['markets', 'total', 'source', 'timestamp'],
        notes: 'Public Polymarket odds, read-only, filtered to conflict, election, sanctions and geopolitics topics.',
      },
      {
        path: '/api/wiki-events',
        method: 'GET',
        summary: 'Wikipedia current-events digest for the day.',
        returns: ['events', 'total', 'day', 'source', 'timestamp'],
      },
      {
        path: '/api/conflicts',
        method: 'GET',
      ...
      },
      {
        ...
        method: 'GET',
        summary: 'Country instability index with a per-component breakdown.',
        returns: ['countries', 'inputs_available', 'exchanges', 'open_exchanges', 'total_exchanges', 'timestamp'],
        notes: 'Scores an editorial baseline plus five live inputs — travel advisories, internet outages, ransomware claims, conflict reporting and seismicity. Each country returns `components` so the score can be audited, and `inputs_available` names sources that failed this run.',
      },
      {
    ...
    endpoints: [
      {
        ...
        notes: 'CISA Known Exploited Vulnerabilities. Records carry no coordinates, so watch rules can match them by entity but not by geofence.',
      },
      {
        path: '/api/ransomware',
        method: 'GET',
        summary: 'Recent ransomware victims by country.',
        returns: ['victims', 'total', 'top_groups', 'source', 'timestamp'],
        notes: 'ransomware.live, keyless. Entries are gang leak-site claims, not confirmed breaches. Only the victim country is known, so markers are jittered off its centroid.',
      },
      {
        path: '/api/tor-exits',
        method: 'GET',
        summary: 'Tor exit-node counts by country.',
        returns: ['countries', 'total', 'relays', 'source', 'timestamp'],
        notes: 'Onionoo, keyless. Aggregated per country because per-relay coordinates are no longer published.',
      },
      {
    ...
    endpoints: [
      {
        path: '/api/scanner',
        method: 'GET',
        summary: 'Runs a scan against a target via the VANTAGE scanner backend.',
        params: [
          {
            name: 'type',
            required: true,
        ...
        env: ['VANTAGE_RECON_ENABLED', 'SCANNER_URL', 'SCANNER_KEY'],
        notes:
          'Disabled by default: returns 503 unless `VANTAGE_RECON_ENABLED=1`. Also 503 when `SCANNER_URL` / `SCANNER_KEY` are unset. `SCANNER_KEY` must equal the backend’s `VANTAGE_KEY`. Every permitted and blocked target is written to the RECON audit trail.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    title: 'AI Analysis',
    blurb:
      'Correlation over feed data you supply, backed by whichever provider `VANTAGE_AI_PROVIDER` names — Ollama (local, default), Claude, or Gemini. Rate limited per IP.',
    endpoints: [
      {
        path: '/api/ai/analyze',
        method: 'POST',
  ...
  },
  {
    ...
    blurb:
      'Multi-turn analyst chat grounded in the layer data you send with each turn. Provider-agnostic — set `VANTAGE_AI_PROVIDER` to ollama, claude or gemini.',
    endpoints: [
      {
        path: '/api/ai/chat',
        method: 'POST',
        summary: 'Answers an analyst question against the supplied intelligence context.',
      ...
      },
      {
        path: '/api/ai/chat',
        method: 'GET',
        summary: 'Reports the selected provider and which providers are configured.',
  ...
  },
  {
    ...
    blurb: 'Scheduled AI read-outs of the live picture, delivered over the alert channels.',
    endpoints: [
      {
        path: '/api/briefs',
        method: 'GET',
        summary: 'Stored briefs, newest first.',
        params: [{ name: 'limit', desc: 'How many briefs to return (1-100).', example: '10' }],
        returns: ['briefs', 'total'],
        requiresAuth: true,
        notes: 'Requires a session when VANTAGE_AUTH_PASSWORD is set.',
      },
      {
        path: '/api/briefs/run',
        method: 'POST',
        summary: 'Generate and deliver a brief now, without waiting for the schedule.',
        returns: ['brief', 'delivered'],
        requiresAuth: true,
        env: ['VANTAGE_DAILY_BRIEF', 'VANTAGE_AI_PROVIDER'],
        notes: 'Answers 503 when no AI provider is reachable. Scheduled generation is off unless VANTAGE_DAILY_BRIEF is set to a local HH:MM.',
      },
    ],
  },
  {
    ...
    endpoints: [
      {
        ...
        summary: 'Streamable HTTP MCP endpoint exposing nine tools over the live picture.',
        returns: ['JSON-RPC 2.0 / SSE'],
        requiresAuth: true,
        env: ['VANTAGE_AUTH_PASSWORD'],
        notes: 'Tools: list_layers, get_layer_data, search_news, get_country_risk, get_alerts, list_watch_rules, get_investigations, get_briefs, create_watch_rule. When a password is set it doubles as the bearer token; the route answers 401 rather than redirecting, since a programmatic client cannot follow a login redirect. create_watch_rule writes and validates its webhook target through the SSRF guard. Connect with: claude mcp add --transport http vantage http://localhost:3000/api/mcp --header "Authorization: Bearer $VANTAGE_AUTH_PASSWORD"',
      },
    ],
  },
  {
    ...
    blurb:
      'Persistent geofence, entity and threshold watches. Rules live in SQLite and deliver to Discord, ntfy, email and generic webhooks.',
    endpoints: [
      {
        path: '/api/watchlist',
        method: 'GET',
      ...
      },
      {
        path: '/api/watchlist',
        method: 'DELETE',
        summary: 'Deletes a rule and forgets its seen-state.',
  ...
  {
    id: 'investigations',
    title: 'Investigations & Dossiers',
    blurb: 'Saved link-analysis graphs and the dossier export built from them.',
    endpoints: [
      {
        path: '/api/investigations',
        method: 'GET',
        summary: 'Lists saved investigations, or fetches one by id.',
        params: [{ name: 'id', required: false, desc: 'Investigation id.' }],
        returns: ['investigations', 'investigation'],
      },
      {
        path: '/api/investigations',
        method: 'POST',
        summary: 'Creates or updates an investigation.',
        returns: ['investigation'],
        notes: 'Body: `{ id?, name, graph: { nodes, links }, notes? }`.',
      },
      {
        path: '/api/report',
        method: 'POST',
        summary: 'Renders an investigation as a Markdown or print-ready HTML dossier.',
        returns: ['…markdown or html'],
        notes:
          'Body: `{ investigationId }` or `{ name, graph }`, plus `format: markdown|html` and `assess: true` to prepend an AI BLUF. `format=html` carries print CSS — use the browser Save as PDF.',
      },
    ],
  },
  {
    id: 'audit',
    title: 'RECON Audit',
    blurb: 'Every active-scan attempt, permitted or blocked, with its target and actor.',
    endpoints: [
      {
        path: '/api/recon-audit',
        method: 'GET',
        summary: 'Returns the RECON audit trail and whether RECON is enabled.',
        params: [{ name: 'limit', required: false, desc: 'Max rows, 1-1000. Default 200.' }],
        returns: ['enabled', 'entries'],
      },
    ],
  },
];
