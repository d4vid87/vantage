...
/**
 * Ask the configured AI provider for the read-out. Returns null on any
 * failure (unconfigured, unreachable, rate-limited) so the caller falls back
 * to the deterministic heuristic and the panel never goes blank.
 */
async function modelOverview(
  ...
  try {
    ...
    const text = await llm.generate({
      system:
        'You are VANTAGE, a terse intelligence analyst. Given structured facts, write a sharp 2-4 sentence situational read-out. No preamble, no markdown headers, no hedging. Lead with the bottom line.',
      prompt: `MODE: ${mode.toUpperCase()}\nBOTTOM LINE: ${digest.summaryLine}\nFACTS:\n${digest.facts.map(f => `- ${f}`).join('\n')}\n\nWrite the read-out now.`,
      maxTokens: 512,
    });
    return text.trim() ? { text: text.trim(), provider: llm.name } : null;
  } catch (e) {
    console.warn('[VANTAGE] AI overview failed, using heuristic:', e);
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  let generatedBy = 'analyst';

  const modelled = await modelOverview(mode, digest);
  if (modelled) {
    overview = modelled.text;
    generatedBy = modelled.provider;
  }
