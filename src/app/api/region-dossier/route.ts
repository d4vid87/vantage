  ...
  try {
    ...
    const geoRes = await fetch(
      `https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=5&addressdetails=1`,
      {
        ...
        headers: { 'User-Agent': 'VantageIntelPlatform/1.0' },
      }
    );
    ...
    const [wikiResult, wdResult] = await Promise.allSettled([
      ...
      (async () => {
        ...
        } catch (e) { console.warn('[VANTAGE] Wikipedia fetch error:', e instanceof Error ? e.message : e); }
        return null;
      })(),
      ...
      (async () => {
        ...
        try {
          ...
          const res = await fetch(
            `https://query.wikidata.org/sparql?format=json&query=${encodeURIComponent(sparql)}`,
            {
              ...
              headers: { 'User-Agent': 'VantageIntelPlatform/1.0', 'Accept': 'application/json' },
            }
          );
          if (res.ok) {
        ...
        } catch (e) { console.warn('[VANTAGE] Wikidata fetch error:', e instanceof Error ? e.message : e); }
        return null;
      })(),
    ]);
