...
function connectAisStream() {
  ...
  // Map AIS ship types to VANTAGE categories
  const getVantageShipType = (typeCode: number) => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (parsed.MessageType === "PositionReport" && parsed.Message?.PositionReport) {
        ...
        existing.type = getVantageShipType(staticData.Type);
      }
