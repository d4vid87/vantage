...
async function getOpenSkyToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.warn('[VANTAGE] OpenSky token failed:', res.status); return null; }
    const data = await res.json();
    if (!data.access_token) {
      console.warn('[VANTAGE] OpenSky token response missing access_token');
      return null;
    }
    ...
    console.warn('[VANTAGE] OpenSky token error:', e);
    return null;
  }
}
...
export async function GET(req: Request) {
  ...
  fetchPromise = (async () => {
    ...
    if (milRes.status === 'fulfilled') {
      if (milRes.value.ok) {
        try {
          ...
          console.warn('[VANTAGE] adsb.fi mil parse error:', e);
        }
      } else {
        console.warn('[VANTAGE] adsb.fi mil feed returned', milRes.value.status);
    ...
    if (osRes.status === 'fulfilled') {
      if (osRes.value.status === 429) {
        ...
        console.warn('[VANTAGE] OpenSky 429 — cooling down 15 min');
        ...
        try {
          ...
          console.warn('[VANTAGE] OpenSky parse error:', e);
        }
      } else {
        // A rejected token surfaces here as a 401 — previously discarded silently.
        console.warn('[VANTAGE] OpenSky returned', osRes.value.status);
        await osRes.value.body?.cancel();
      }
    }
    ...
    if (!openSkyWorked) {
      source = 'regional';
      console.warn('[VANTAGE] no OpenSky snapshot — falling back to adsb.fi regional sweep');
      ...
      if (allRaw.length === 0) {
        console.error(
          '[VANTAGE] every flight provider returned zero aircraft — ' +
  ...
  try {
    ...
    if (cachedData) {
      console.warn('[VANTAGE] Returning stale flight cache as fallback');
