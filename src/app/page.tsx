...
function useIsMobile() {
  ...
  useEffect(() => {
    const check = () => {
      ...
      setIsMobile(w < 1024 || (h < 500 && w < 1024));
    };
    check();
...
export default function Dashboard() {
  ...
  useEffect(() => {
    (window as unknown as { vantageWatchFlight?: (f: WatchedFlight) => void }).vantageWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const [idleRotation, setIdleRotation] = useState(false);
  const [vantageTheme, setVantageTheme] = useState<'core'|'ghost'>('core');

  useEffect(() => {
    document.body.className = vantageTheme === 'core' ? '' : `theme-${vantageTheme}`;
  }, [vantageTheme]);
  ...
  // Restore explicit shared-link choices without overriding the global opening view.
  useEffect(() => {
    ...
    setIdleRotation(localStorage.getItem('vantage-idle-rotation') === 'true');
  ...
  // URL state: persist explicit layer choices; the default camera stays global.
  ...
  useEffect(() => {
    const open = () => { closeWorkspacePanels('Search'); setShowDesktopSearch(true); };
    window.addEventListener('vantage-open-search', open);
    return () => window.removeEventListener('vantage-open-search', open);
  ...
  // ── Drawing / AOI ──
  // VantageMap already owns the draw interaction and the polygon rendering;
  ...
  const handleExportGeoJSON = useCallback(() => {
    downloadFile(
      `vantage-aoi-${new Date().toISOString().slice(0, 10)}.geojson`,
  ...
  const refreshFeed = useCallback(async (feed: MapFeed, force = false) => {
    ...
    try {
      ...
      feedState.current[feed.key] = {
        key: feed.key, interval, lastSuccess: Date.now(), lastAttempt: Date.now(), loading: false,
        source: typeof json.source === 'string' ? json.source : undefined,
        observedAt: typeof json.observed_at === 'string' ? json.observed_at : typeof json.timestamp === 'string' ? json.timestamp : undefined,
        retrievedAt: typeof json.retrieved_at === 'string' ? json.retrieved_at : new Date().toISOString(),
        availability: json.availability === 'stale' ? 'stale' : 'current',
      };
      setData(dataRef.current);
    } catch (e) {
      feedState.current[feed.key] = { ...feedState.current[feed.key], loading: false, availability: feedState.current[feed.key]?.lastSuccess ? 'stale' : 'unavailable', error: e instanceof Error ? e.message : 'Request failed' };
  ...
  // ── VANTAGE SDK — Intelligence Fusion Layer ──
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── VANTAGE title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'VANTAGE'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <ErrorBoundary name="Map">
        <VantageMap 
          ...
          rotationBlocked={Boolean(mobilePanel || showCopilot || showWatchlists || showBriefs || showFeedHealth || showSavedViews || showRisk || showGraph || showIntel || showMarkets || showAlerts || showDrawing || showDirections || showSpaceCam || showDesktopSearch || showArcGIS || showRemote)}
          lowPower={lowPower}
          theme={vantageTheme}
      ...
      >
        ...
        <div className="flex items-center gap-[3px] p-[3px] pointer-events-auto rounded-xl border border-[var(--border-primary)] bg-[var(--bg-panel)] backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.55)]">
          ...
          <ViewSegment layoutId="view-style" active={mapStyle === 'satellite'} onClick={() => setMapStyle('satellite')} title="Basemap imagery — not a live satellite observation" icon={Satellite} label="IMAGERY" />
          <div className="w-px h-5 mx-1 bg-[var(--border-secondary)]" />
          <button
            type="button"
            className="map-rotation-toggle"
            aria-pressed={idleRotation}
            title="Rotate the globe after 20 seconds of inactivity"
            onClick={() => setIdleRotation(value => {
              localStorage.setItem('vantage-idle-rotation', String(!value));
              return !value;
            })}
          >
            Idle rotation
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={vantageTheme} setTheme={setVantageTheme} capabilities={capabilities} />}
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-vantage-pulse" />
                ...
                <div className="px-3 pb-3">
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} isMobile={true} theme={vantageTheme} setTheme={setVantageTheme} capabilities={capabilities} />
                      <div className="mt-8">
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 vantage-glow">
