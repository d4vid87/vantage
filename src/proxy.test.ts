...
it('allows only the configured tick credential through the auth proxy',async()=>{process.env.VANTAGE_TICK_SECRET='cron-key';expect((await proxy(new NextRequest('http://localhost/api/alerts/tick',{headers:{'x-vantage-tick-key':'cron-key'}}),event)).status).toBe(200);expect((await proxy(new NextRequest('http://localhost/api/alerts/tick',{headers:{'x-vantage-tick-key':'wrong'}}),event)).status).toBe(401);delete process.env.VANTAGE_TICK_SECRET;});
