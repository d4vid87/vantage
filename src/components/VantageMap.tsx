  ...
  useEffect(() => {
    ...
    for (const canvasContextAttributes of attributeFallbacks) {
      try {
        ...
        console.warn('[VANTAGE] WebGL context rejected, retrying with weaker attributes:', e instanceof Error ? e.message : e);
      }
    }
    if (!map) return;
    ...
    map.on('load', () => {
      ...
      // Create icons — VANTAGE Unified Palette
      ...
      if (process.env.NODE_ENV === 'development') (window as any).__vantageMap = map;
    });
    ...
    ['fl-commercial','fl-private','fl-jets','fl-military'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(255,255,255,0.08);">
          ...
          <button onclick="window.vantageWatchFlight && window.vantageWatchFlight({ icao24: '${idSafe(p.icao24||'')}', callsign: '${idSafe(cs)}' })" style="width:100%;margin-top:8px;padding:6px 12px;background:rgba(0,229,255,0.10);border:1px solid rgba(0,229,255,0.35);color:#7FE9FF;font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:bold;letter-spacing:0.1em;border-radius:4px;cursor:pointer;">+ WATCH THIS AIRCRAFT</button>
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://github.com/d4vid87/vantage';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'VANTAGE')}</a></div>
          </div>
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('vantage-buildings')) {
          map.addSource('vantage-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('vantage-3d-buildings')) {
          map.addLayer({
            id: 'vantage-3d-buildings',
            source: 'vantage-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('vantage-3d-buildings')) map.removeLayer('vantage-3d-buildings');
      }
    } catch (e) {
      console.warn('[VANTAGE] 3D terrain toggle error:', e);
    }
...
export default memo(VantageMap);
